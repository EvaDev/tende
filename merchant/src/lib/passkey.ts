// lib/passkey.ts
// WebAuthn passkey ceremonies for merchant operators. Unlike the consumer app,
// this passkey does NOT back a Safe wallet — it's just the operator's login
// credential, scoped server-side to one merchant_members row. Mechanically
// identical to consumer/src/lib/passkey.ts (same ES256/P-256 requirement, same
// server-side verifyAssertion), copied here to keep this app dependency-free
// of the consumer app.
...
export async function createPasskey(opts: {
  ...
  const cred = await navigator.credentials.create({
    publicKey: {
      ...
      pubKeyCredParams: [{ type: 'public-key', alg: -7 }], // ES256 / P-256 — required by the server's extractP256FromSpki
  ...
  if (typeof resp.getPublicKeyAlgorithm === 'function' && resp.getPublicKeyAlgorithm() !== -7) {
    throw new Error('This passkey is not P-256 (ES256) and is not supported.');
  }
