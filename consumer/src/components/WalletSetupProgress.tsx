...
export default function WalletSetupProgress({ steps, currentId }: Props) {
  const currentIdx = Math.max(0, steps.findIndex(s => s.id === currentId));
  const pct = Math.round(((currentIdx + 0.35) / steps.length) * 100);
  const r = 42;
  const c = 2 * Math.PI * r;
  ...
  return (
    <div className="bg-brand-card border border-brand-accent/20 rounded-2xl p-5 space-y-4 shadow-sm">
      <div className="flex items-center gap-4">
        ...
        <div>
          <p className="font-semibold text-brand-accent">Setting up your account</p>
          <p className="text-xs text-brand-accent/60 mt-1">This can take up to a minute — please keep this screen open.</p>
        </div>
      </div>
