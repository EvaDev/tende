...
import { LogOut, Shield, Copy, CheckCheck } from 'lucide-react';
    ...
    <>
      <div className="flex flex-col min-h-dvh pb-24">
        <div className="px-6 pt-12 pb-4">
          <h2 className="text-2xl font-bold text-brand-accent">Account</h2>
        </div>
        <div className="px-6 space-y-4">
          ...
          <div className="bg-brand-card border border-brand-accent/20 rounded-2xl p-5 space-y-3 shadow-sm">
            ...
            <div className="grid grid-cols-2 gap-2 text-xs">
              <div className={`flex items-center gap-2 px-3 py-2 rounded-xl ${profile?.kyc.allowsRemittance ? 'bg-green-100 text-green-700' : 'bg-brand-accent/10 text-brand-accent/50'}`}>
                {profile?.kyc.allowsRemittance ? '✓' : '✗'} Remittance
              </div>
              <div className={`flex items-center gap-2 px-3 py-2 rounded-xl ${profile?.kyc.allowsUsdSavings ? 'bg-green-100 text-green-700' : 'bg-brand-accent/10 text-brand-accent/50'}`}>
          ...
          <button
            ...
            className="w-full flex items-center justify-center gap-2 py-4 rounded-2xl border-2 border-brand-danger/40 text-brand-danger font-medium active:scale-95 transition-transform"
          >
